...
- **`BWA_INDEX`** no longer runs `bwa index` locally — it downloads the fasta *and* the prebuilt `.amb/.ann/.bwt/.pac/.sa` from Zenodo (DOI `10.5281/zenodo.21682938`, "GORG Dark - Reference Contaminant Dataset") and verifies each against Zenodo's published MD5 before extracting. Two non-obvious things about that download, both verified empirically (not documented anywhere):
...
- `CONTAM_READ_FINDER` and `CHECKM_v1_1_9` both have `memory '11.GB'` and `maxForks 1` — Docker Desktop's VM has a real memory ceiling (raised from its 7.75GB default to make this reference usable at all), and these are the two steps that come close to it. `maxForks 1` on each stops two samples' tasks from stacking their memory demand past that ceiling, since Nextflow sizes its own scheduling against the *host's* memory, not Docker Desktop's VM allocation.
